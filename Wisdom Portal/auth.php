...
$auth_name = $authors[$authid];
$aut_time = file("aut_time.txt");
$times = file("times.txt");
$timeid = rtrim($aut_time[$authid]);
$auth_time = $times[$timeid];
$authorsdes = file("authorsdes.txt");
$auth_descr = $authorsdes[$authid];
$notes = file("notes.txt");
$auth = file("aut_not.txt");
$posfin = $auth[$authid];
if ($authid == 0) {$posstart = 0;}
else {$posstart = $auth[$authid-1]+1;};  // нашли позиции старта и конца заметок для данного автора
$pagecount = ceil(($posfin-$posstart)/10);
?>
<html>
<head>
	<title><? echo $auth_name." высказывания, философия"?>
	</title>
    <script type="text/javascript" src="jquery.js"></script>
	<link rel="stylesheet" type="text/css" href="css.css">
	<meta charset="utf-8">
    <meta name="keywords" content='<? echo $auth_name." высказывания о жизни, любви, браке, ".$auth_name." афоризмы, ".$auth_name." мудрость.".$auth_name." философия, цитаты"; ?>, античная мудрость, древние философы, великие высказывания'>
    <meta name='description' content='<? echo $auth_name." философия, афоризмы, мудрые высказывания о жизни";?>'>
...
<!-- /Yandex.Metrika counter -->
	<div class="back">
		<div class="main">
            <div id='evr'>
				<div class="logo">
					<a href='main.php' title='На главную' ><img alt='Книга мудрости'  src="logo.png"></a>
        ...
        <span class="small"><i>Вся мудрость древности и современности - здесь!</i></span>
                     ...
                     <div class='top_sel'>
                    <a class='und' href='main.php'>На главную</a>
                               </div>
...
(adsbygoogle = window.adsbygoogle || []).push({});
            ...
            <div id='downsided' class='upped' style='top:90px'>
                <div class='auth_descr'><div class='descr_tit'><? echo $auth_name ?></div>
                <div class='descr_time'><? echo $auth_time ?></div><div id='descr'><? echo $auth_descr ?></div></div>
            <div id='notes'>
                <?
                if ($page == "") {
                    $page = 0;};
                ...
                <div id='pages'>
                    ...
                    if ($pagecount > 1) {
                        while ($c <= $pagecount) {
                            if ($c == $page+1) {
                                echo "<a href='auth.php?id=".$authid."&page=".($c-1)."'><div class='page' style='background:#FFEFD5'><span>".$c."</span></div></a>";
                                $c++;
                            }
                            else {
                            echo "<a href='auth.php?id=".$authid."&page=".($c-1)."'><div class='page'><span>".$c."</span></div></a>";
                            $c++;}
                        }
                    }
                    ?>
            ...
            <div id='r_block'>
                ...
                <div id='r_block_cont'>
                    <strong><? echo $auth_name." высказывания о жизни, любви, семье<br>".$auth_name." история, биография<br>".$auth_name."  философия<br>"."Философ ".$auth_name." цитаты" ?></strong>
                </div>
            </div>
